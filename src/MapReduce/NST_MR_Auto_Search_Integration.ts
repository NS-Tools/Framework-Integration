...
namespace NST_MR_Auto_Search_Integration {
	...
	export function getInputData(_context: EntryPoints.MapReduce.getInputDataContext) {
		log.debug('Getting Input Data');
		const results = [];
		const mySearch = LazySearch.from(
			search.create({
				type: search.Type.CUSTOMER,
				filters: [['internalid', 'is', CONSTANTS.CUSTOMER_ID]],
				columns: ['internalid', 'companyname', 'email', 'phone', 'datecreated', 'lastmodifieddate'],
			}),
		);
